...
qsub -q ${queue_name} -N novoalign.${sample_name} -l h_vmem=${novo_mem}G -pe multi_thread ${novo_cpu} -M ${email_contact} -m beas ${ngs_pipeline}/ngs_novoalign.illumina.${qual_type}.PE.sh \
fastqprefix=${fastq_prefix} samplename=${sample_name} alnsam=${sample_name}. wrkingdir=${sample_dir};
