...
#############################################################################################
## echo this shit out of everythin #########################################################
###########################################################################################
...
if [ ${mPE} -eq 1 ]; then
qsub -q ${queue_name} -N novoalign.${sample_name} -l h_vmem=${novo_mem}G -pe multi_thread ${novo_cpu} -M ${email_contact} -m beas ${ngs_pipeline}/ngs_novoalign.${qual_type}.PE.sh ${fastq_prefix} ${sample_name} ${sample_dir};
else
qsub -q ${queue_name} -N novoalign.${sample_name} -l h_vmem=${novo_mem}G -pe multi_thread ${novo_cpu} -M ${email_contact} -m beas ${ngs_pipeline}/ngs_novoalign.${qual_type}.SE.sh ${fastq_prefix} ${sample_name} ${sample_dir};
fi
...
qsub -q ${queue_name} -N AddOrReplaceReadGroups.${sample_name} -hold_jid SortSam.${sample_name} -l h_vmem=${sge_h_vmem}G -pe multi_thread 1 -M ${email_contact} -m beas ${ngs_pipeline}/ngs_AddOrReplaceReadGroups.sh ${sample_name} ${sample_dir} ${sample_temp} ${mRGID} ${mRGLB} ${mRGPL} ${mRGPU} ${mRGSM} ${mRGCN} ${mRGDS} ${mRGDT};
...
${ngs_pipeline}/ngs_AnalyzeCovariates_before_and_after_BQSR.sh ${sample_name} ${sample_dir} ${sample_temp};
