...
#----------------------------------------------------------------------#
# 0. Check for proper number of command line args and get settings
#----------------------------------------------------------------------#
## TO DO
...
if [ ${mPE} -eq 1 ] ; then
	...
	qsub -q ${queue_name} -N novoalign.${sample_name} ${ngs_pipeline}/ngs_novoalign.${qual_type}.PE.sh ${fastq_prefix} ${sample_name} ${sample_dir};
	
else

	qsub -q ${queue_name} -N novoalign.${sample_name} ${ngs_pipeline}/ngs_novoalign.${qual_type}.SE.sh ${fastq_prefix} ${sample_name} ${sample_dir};
...
qsub -q ${queue_name} -N sam2bam.${sample_name} -hold_jid novoalign.${sample_name} ${ngs_pipeline}/ngs_sam2bam.sh ${sample_name} ${sample_dir};
...
qsub -q ${queue_name} -N SortSam.${sample_name} -hold_jid sam2bam.${sample_name} ${ngs_pipeline}/ngs_SortSam.sh \
${sample_name} \
${sample_dir} \
${sample_temp};
...
qsub -q ${queue_name} -N AddOrReplaceReadGroups.${sample_name} -hold_jid SortSam.${sample_name} ${ngs_pipeline}/ngs_AddOrReplaceReadGroups.sh \
${sample_name} \
${sample_dir} \
${sample_temp} \
...
qsub -q ${queue_name} -N MarkDuplicates.${sample_name} -hold_jid AddOrReplaceReadGroups.${sample_name} ${ngs_pipeline}/ngs_MarkDuplicates.sh \
${sample_name} \
${sample_dir} \
${sample_temp};
...
##
##qsub -q ${queue_name} -N rmvIntermediateSAMs.${sample_name} -hold_jid MarkDuplicates.${sample_name} ${ngs_pipeline}/ngs_rmvdIntermediateSAMs.sh \
##${sample_name} \
##${sample_dir} \
##${sample_temp};
