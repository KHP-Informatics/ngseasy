...
ln -s ${FASTQDIR}/${FASTQ1} ${SOUT}/fastq/${FASTQ1}
ln -s ${FASTQDIR}/${FASTQ2} ${SOUT}/fastq/${FASTQ2}
...
echo " Run Pre-Alignment QC on raw Fastq files "
...
qcdPeFASTQ1=${rawFASTQ1}_1.filtered.fq.gz
qcdPeFASTQ2=${rawFASTQ2}_2.filtered.fq.gz
# Trimmomatic unpaired ouput
qcdSeFASTQ1=${rawFASTQ1}_1.unpaired.fq.gz
qcdSeFASTQ2=${rawFASTQ2}_2.unpaired.fq.gz
...
echo " Run Pre-Alignment QC on Filtered/Trimmed Fastq files "
...
echo " Finding Covered Intervals : minimum coverage of 4 "
...
echo " Run CollectTargetedPcrMetric "

if [ ${NGS_TYPE} == "TGS" ];then
echo " Finding TGS coverage over target regions in ${BED_ANNO} "
...
echo " Finding WEX Coverage over annotated exons/genes from ensembl "
java -XX:ParallelGCThreads=${NCPU} -Xmx6g -jar /usr/local/pipeline/picardtools/picard-tools-1.115/CollectTargetedPcrMetrics.jar \
TMP_DIR=${SOUT}/tmp \
VALIDATION_STRINGENCY=SILENT \
MAX_RECORDS_IN_RAM=100000 \
...
echo " Finding WGS Coverage over 500bp windows "
java -XX:ParallelGCThreads=${NCPU} -Xmx6g -jar /usr/local/pipeline/picardtools/picard-tools-1.115/CollectTargetedPcrMetrics.jar \
TMP_DIR=${SOUT}/tmp \
VALIDATION_STRINGENCY=SILENT \
MAX_RECORDS_IN_RAM=100000 \
...
echo " Basic Post Alignmnet QC Complete "


##---------------------- GATK VARIANT CALLING SINGLE SAMPLE ----------------------##
echo " Start SNP and Small INDEL Calling "

KNOWN_SNPS_b138=/usr/local/pipeline/gatk_resources/dbsnp_138.b37.vcf

echo " Starting Variant Calling using Freebayes "
...
echo " Starting Variant Calling using Platypus "
if [ ${NGS_TYPE} == "TGS" ];then
...
echo " Running GATK UnifiedGenotyper "
